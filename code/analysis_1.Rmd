...
sims %>%  filter(!is.na(sim)) %>% 
  ...
  filter(type1!=type2) %>% # can only compare across blocks in some cases, so only cross ever
    mutate(blocks=case_when(
      type1=="block 1" & type2=="block 2"~ "1-2",
      type1=="block 2" & type2=="block 1"~ "1-2",
      type1=="block 1" & type2=="block 3"~ "1-3",
      type1=="block 3" & type2=="block 1"~ "1-3",
      type1=="block 2" & type2=="block 3"~ "2-3",
      type1=="block 3" & type2=="block 2"~ "2-3"
      )) %>% 
  ggplot(aes(x=str_c(same_target,"\n",same_child), y=sim, color=blocks)) +
