...
player_ids <- str_c("id", 3:12) # Ids 1, 2 were adult pilot test 
```
...
players <- read_csv(here(player_source)) |> filter(id %in% player_ids) |> select(id, playerId=`_id`) 

games <- read_csv(here("data/games.csv"), col_types="ccc") |> mutate(offset_sec=lubridate::ms(offset) |> lubridate::period_to_seconds()) |> select(-offset)
...
timing <- raw_data |> group_by(game) |> mutate(cum_time=cumsum(time), cum_sec=cum_time/1000+3*trialNum)
...
readxl::read_xlsx(here("data/clean_transcripts", str_c(game_num,".xlsx"))) |> 
...
timed_transcripts <- transcripts |> mutate(start=start-offset_sec, end=end-offset_sec)
```
