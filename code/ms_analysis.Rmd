...
#data |> filter(is.na(response))

#data |> left_join(words) |> filter(is.na(words)) |> filter(!is.na(repNum))
```
...
```{r}
# get racial/ethnic demographics  breakdowns
demogs <- read_csv(here(data_loc_1, "clean_demog.csv")) |>
  bind_rows(read_csv(here(data_loc_2, "clean_demog.csv"))) |>
  bind_rows(read_csv(here(data_loc_3, "clean_demog.csv"))) 

relevant_participants <- read_csv(here("data/processed_for_paper/joint_with_demog.csv")) |> distinct(expt, speaker, game) |> rename(id=speaker) |> left_join(demogs)

relevant_participants |> group_by(expt, gender) |> tally()

foo <- relevant_participants |> group_by(expt, ethnicity, hispanic) |> tally() |> filter(expt==3)
...
transcripts |>
  ...
  select(game, expt, trial, speaker_transcript = speaker, role, text) |>
  filter(role == "S") |>
  full_join(joint) |> filter(correct==1) |> filter(expt==3) |> filter(type!="practice") |>  View()
...
sim_acc_to_next |> ggplot(aes(as.character(earlier), sim, color = as.character(correct))) +
  ...
  theme(legend.position = "bottom")+ggthemes::scale_color_solarized()
```
...
sim_across_mod_meta <- brm(sim ~ block1 + (block1 | target)+ (block1|expt),
                        data = sim_across_data |> filter(expt=="2"|expt=="3"),
...
acc_mod_demog <- brm(
  correct ~ trial +
    ...
    (trial | gameId) + (trial | target) + (trial|expt),
  family = bernoulli(link = "logit"),
  data = centered_demog,
