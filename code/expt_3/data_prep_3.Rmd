...
less_raw_data <- raw_data %>%
  ...
  write_csv(here("data/expt_2/clean_data.csv"))
```
...
wrangle_transcript <- function(game_num) {
  read_xlsx(here("data/expt_2/raw_transcripts", str_c(game_num, ".xlsx"))) |>
...
transcripts <- read_csv(here("data/expt_2/link_transcripts.csv")) |>
...
write_csv(transcripts, here("data/expt_2/transcripts.csv"))
```
...
```{r}
transcript <- read_csv(here("data/expt_2/transcripts.csv"))

dat <- read_csv(here("data/expt_2/clean_data.csv")) %>% select(trial = trialNum, gameId, target, distractor)
transcript %>%
  ...
  left_join(dat) %>%
  write_csv(here("data/expt_2/pre_sbert.csv"))
```

```{r}
library(reticulate)
np <- import("numpy")
mat <- np$load(here("data/expt_2/post_sbert.npy"))
saveRDS(mat, here("data/expt_2/post_sbert.RData"))
```

```{r}
sbert <- read_csv(here("data/expt_2/pre_sbert.csv")) %>% bind_cols(read_rds(here("data/expt_2/post_sbert.RData")) %>%
  as_tibble())
...
similarities <- M_mat %>%
  ...
  write_rds(here("data/expt_2/similarities.rds"))
```
