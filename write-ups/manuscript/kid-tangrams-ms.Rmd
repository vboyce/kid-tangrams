...
title             : |
  ...
  | to communicate about novel referents (TODO?)
shorttitle: Preschoolers can form conventional pacts (TODO?)
author: 
  - name          : "Veronica Boyce"
    affiliation   : "1"
...
authornote: |
  TODO credit taxonomy, acknowledgements, etc 

abstract: > 
  Learning language requires learning not only the content of language, but also how to use language to communicate. Iterated reference games provide a window into such skills, requiring rich communication as participants converge on mutually understandable names for initially novel referents. Some early experiments are interpreted as evidence that 4-5-year-old children cannot generate the mutually understandable names needed to succeed in an iterated reference game. Here, we revisit young children's referential communicative abilities using a simpler, child-friendly paradigm. Across TODO pairs of children, we found that 4-5-year-olds successfully established reference with each other. Children were TODO% accurate, and they often used descriptions similar to their partner's. These findings suggest that children’s capacity to construct effective referring expressions in novel contexts emerges earlier than once thought, consistent with the view that children show early pragmatic competence in supportive contexts.
...
More recently, though no evidence has directly contradicted @glucksberg1966, a number of other studies have revealed early emerging skills in the preschool years that support the use of referential communication. Preschool-aged children are faster to select a target when it is referred to in a consistent way [@graham2013; @matthews2010]. When a change in visual context renders previous descriptions overinformative, 6-year-olds (but not 4-year-olds) are more likely to use consistent referential expressions when they keep talking to the same partner, but switch to simpler terms with a new partner [@koymen2014] These studies suggest that young children are sensitive to the norm of consistent descriptions in cooperative communication. 
...
We excluded trials where the "teller" did not produce a description, or where all description was unintelligible and impossible to transcribe. After exclusions, we had `r joint_data |> filter(expt=="1") |> nrow()` trials remaining. 

Statistical analyses were run in brms [@burkner2018] with weakly informative priors. We report estimates and 95% credible intervals. The experimental set-up, analysis code, de-identified transcripts, and performance data for all experiments is available at [anonymized repo](https://osf.io/7d3ne/?view_only=d094ccd6438741bfbcaa7fde9fbb56b2).


## Results
### Accuracy and speed

```{r fig-accuracy, fig.env="figure", fig.pos = "t", fig.align = "center", out.width="100%", fig.width=7, fig.height=3, fig.cap = "Children's accuracy at selecting the correct target over time. Error bars are bootstrapped 95% CIs with a linear trend line overlaid. \\label{acc}" }
joint_data |>
...
```{r description, fig.env="figure", fig.pos = "t", fig.align = "center", out.width="100%", fig.width=7, fig.height=3, fig.cap = "Length of description produced by the teller each trial. Grey dots are individual data points, colored dots are per trial means with bootstrapped 95% CIs. \\label{length}" }
joint_data |> 
...
TODO this description metric is just the length of the description, taking out repetition/rephrasing from the child. We could also include the robustness check which is the length of all the (on-topic) teller-guesser interaction (including repeating/rephrasing, questions, I don't knows, etc), which is also very flat. 


[TODO maybe move this paragraph] Some examples to illustrate the variety of effective descriptions children employed are shown in Table \ref{example} (these specific examples are from Experiment 2, but both experiments had similar distributions of descriptions). 

### Convergence 

```{r sims, fig.env="figure", fig.pos = "t", fig.align = "center", out.width="100%", fig.width=7, fig.height=3, fig.cap = "Semantic similarity between pairs of descriptions from different sources. Dots are means and lines are bootstrapped 95% CIs.  \\label{sim}" }
...
TODO consider dropping this because it's not that strong an analysis? 
...
```{=latex}
\begin{table}
	\caption{Example descriptions children successfully used to identify different target images in Experiment 2. TODO UPDATE EXAMPLES TO HAVE SOME FROM EXPT 3
}
	\label{example}
...
```{r sim2, fig.env="figure", fig.pos = "t", fig.align = "center", out.width="100%", fig.width=7, fig.height=3, fig.cap = "Semantic similarity between descriptions from earlier blocks (1-3) and the last block in Experiments 2 and 3.  Heavy dots are means with bootstrapped 95% CIs; light dots are individual values. \\label{tolast}" }
joint_similarities |> 
  filter(game1==game2) |> 
  filter(later==3) |> 
...
## Methods
As Experiment 2 was very similar to Experiment 2, we focus on the changes made compared to Experiment 2. Experiment 3 was pre-registered at TODO [anonymized link]. 

The procedure for Experiment 3 matched Experiment 2, and the only change was in the stimuli used.


### Participants
Experiment 3 was run between XX and YY, at the same preschool as Experiments 1 and 2. Because the stimuli changed, we allowed children who had participated in a prior experiment to participate again, and ZZ participants in experiment 2 were also in experiment 3. TODO pairs of children completed all 16 critical trials, and TODO others completed ... . Our target age range was 4 and 5-year-olds. Of the TODO children, QQ were girls, and the children had a median age of ZZ months, and a range of XX-YY months. An additional XX games were excluded for being pilot runs, including repeat participants, not completing enough trials, or including a 3-year-old participant.  

### Materials
A different set of critical images were used (see Figure TODO). These were chosen from a set of commonly used tangram images (CITATIONS), to be fairly distinct from each other. We also switched the stimuli for the 3rd and 4th practice trials to be more abstract, so children would not be surprised by the critical stimuli not having convention names (TODO figure). 
...
In Experiment 3, children's accuracy was above chance (Odds Ratio: `r stats_text(acc_mod_3,1)`) and relatively stable over time (OR of one trial later: `r stats_text(acc_mod_3, 2)`, Figure \ref{acc}). Children's overall accuracy was lower in Experiment 3(`r round(acc_pct |> pluck(2,3))`%) than in the prior experiments (`r round(acc_pct |> pluck(2,2))`%), potentially indicating that the stimuli were more difficult. 
...
The average length of descriptions on the first trial was `r stats_text(description_mod_3, 1)` words and description length was relatively stable over time (change of `r stats_text(description_mod_3, 2)` words / trial, Figure \ref{length}). TODO after we double check data again, but might be overall more words than other expts? 

### Convergence 

TODO reverify everything after full sample 
...
Over the first three blocks, descriptions became increasingly similar to the last block description (`r sim_to_last_3_results |> stats_text(2,3)`). Over time, the semantic similarity between adjacent block utterances increased slightly (`r sim_to_next_2_results |> stats_text(2,3)`). 

Unlike in Experiment 2, we did not find that different games became less similar over time (`r sim_across_3_results |> stats_text(2,3)`). 
TODO interpretation
...
As the experiments were all similar to one another, we re-ran models pooling the data across the three experiments, with experiment number as a random effect. Pooling across the three experiments, children's accuracy was above chance (OR: `r stats_text(acc_mod_meta,1)`) and accuracy numerically increased over the course of the game, although the credible interval included 0 (OR of one trial later: `r stats_text(acc_mod_meta, 2)`). 

TODO decide whether to include speed here again. 
...
For the other semantic analyses, we pooled across experiments 2 and 3 which had 4 blocks. In the combined model, we found that the similarity the last block utterance increased across blocks (`r stats_text(sim_to_last_mod_meta, 2,3)`) and was higher when the same child produced both utterances (`r stats_text(sim_to_last_mod_meta, 3,3)`). In a combined model, similarity to the next block utterance also increased across blocks (`r stats_text(sim_to_next_mod_meta, 2,3)`) and was higher when the same child produced both utterances (`r stats_text(sim_to_next_mod_meta, 3,3)`). Across the two experiments, utterances from different pairs to the same stimulus tended to decrease in similarity across blocks (`r stats_text(sim_across_mod_meta, 2,3)`). 

While some of these effects were small and thus not all effects were robustly detectable in every experiment, the overall pattern of results is consistent in direction with the patterns of convergence and divergence reported in reference games between adults [@boyce2024]. 


## Sensitivity to prior result
...
Children in the late preschooler years are undergoing a time of rapidly developing linguistic and cognitive skills, so children may show age-based differences in how well they complete tasks that use language or executive functioning [citation needed]. Working memory and other executive capacities are developing across childhood [@diamond2013]. Additionally, there may be gender differences at this age. Young girls have on average an advantage in early linguistic skills, showing faster earlier vocabulary growth that boys [@marchman; @rinaldi2023], and this linguistic advantage continues until .... TODO

We did not design these experiments to test for the affect of children's age or gender. Children were not matched with their partner based on age and gender, so different ages (within the 4-5 year old target age range) sometimes played together, and children sometimes played with same-gender and sometimes with different-gender partners. We consider the effects of participant age and gender as an exploratory measure. 

```{r age-acc, fig.env="figure", fig.pos = "t", fig.align = "center", out.width="100%", fig.width=7, fig.height=2, fig.cap = "Accuracy of selections across all experiments, graphed by the mean age of child in that game, and colored by the number of girls in the pair. Each dot is a game, and linear fits are overlayed. \\label{ageacc}" }
joint_data |>
...
```{r age-words, fig.env="figure", fig.pos = "t", fig.align = "center", out.width="100%", fig.width=7, fig.height=2, fig.cap = "Number of words of description produced by the teller across all experiments, graphed by the age of teller in that game, and colored by teller gender. Each dot is a game, and linear fits are overlayed. \\label{agewords}" }
joint_data |>  ggplot(aes(x = speaker_age, y = words, color = speaker_gender)) +
  geom_smooth(method = "lm", show.legend=F)+
  geom_jitter(data=joint_data |> group_by(speaker_age, speaker, expt, game, speaker_gender) |> summarize(words=mean(words, na.rm=T)))+
  scale_color_manual(values=c("#268BD2FF", "#DC322FFF"))+
  labs(x="Teller Age (months)", y="Words", color="Teller gender")
```
...
The effects of children's age and gender on their performance are shown in Figure \ref{ageacc}. We ran a mixed effects logistic regression, where we considered possible effects of the age of each child (centered at mean age of all the children who participated) and the gender of each child on the accuracy of their selections^[`r form(acc_demog_form)`]. Accuracy was higher for older children (OR for one month older speaker: `r stats_text(acc_demog_meta, 3,2)`, OR for one month older listener: `r stats_text(acc_demog_meta, 2,2)`). Accuracy was also higher when the listener was a girl (OR: `r stats_text(acc_demog_meta, 4,2)`), but was not credibly higher when the speaker was a girl (OR: `r stats_text(acc_demog_meta, 5,2)`). 

TODO be consistent about teller/guesser versus speaker/listener

We also tested whether children's age or gender affected how many words they produced in their descriptions (Figure \ref{agewords}). We ran a mixed effects linear regression, where we considered the ages and genders of both children as possible predictors of description length^[`r form(description_demog_form)`]. Older children tended to produce longer descriptions (effect of one month older `r stats_text(description_demog_meta,3,2)`), but there was no effect of age of listener (`r stats_text(description_demog_meta, 2,2)`). For gender, there was no effect of teller gender on description length (girls produced `r stats_text(description_demog_meta, 5,2)` words compared to boys), but tellers produced longer descriptions for boy guessers (`r stats_text(description_demog_meta, 4,2)`). There were not substantial effects of trial order or interactions between demographics and trial order. 

It is curious that listener gender but not speaker gender affected description length, but for this task, it is unclear how description length would be expected to map onto linguistic sophistication for this task and demographic. At one end, longer descriptions may provide more information than short non-specific labels such as "person" or "diamond". On the other, succinct descriptions that reflect distinguishing features ("mountain", "man with backpack") are better than long descriptions about the component shapes ("there's a triangle and then a square and another square...").

For accuracy, the age and gender effects could be reflective of any of a number of developing capacities, including linguistic sophistication, improving executive functioning and impulse control, and better perspective taking and theory of mind. 
...
* do want to make the point that there's really wide variability (consistent with older kids and also adults for that matter) -- some pairs one kid does the describing including of options and the other chooses

* may want to be clear that structuring the interaction in an effective way is a skill that not all the kiddos have -- some relied more on the experimenter saying whose turn and needed more prompting with "do you want to give more hints" or "do you want to ask a question". whereas other groups rapidly produced descriptions and then either clicked or asked for (and got) clarification (either completely on their own, or after the option was questions offered as a possibility). 
that said, even when children were struggling, some of their attempts such as gesturing or turning ipads or asking (can you show me) are indicative of attempts at conversational monitoring and solving the communication problem (albeit not in ways that followed the "rules" of the game)
where we see what looks like failures it may be hard to tell what part isn't working (is it knowing what i know, or what other knows, or cognitive control, or producing questions, or sampling)
...
Across 3 experiments and TODO pairs of 4 and 5-year-old children, we tested how well children could produce referential expressions that allowed their partner to find a matching abstract shape. Children varied substantially in what sorts of descriptions they produced, but overall accuracy was high (TODO%), indicating that children were generally able to produce adequate descriptions. Additionally, children's utterances showed signs of converging toward conceptual pacts. While this task was substantially scaled down relative to measures used for adult competence, it does suggest that the relevant communication skills are present at least in rudimentary form by the end of the preschool years. 
...
The generalizability of our results is limited by the target population, the target images, and the task structure. We sampled a convenience population of children at a university nursery school. We used two sets of 4 tangram images, which may be easier to distinguish and have higher codability than other target images used in adult reference games. We specifically targeted children's abilities to construct referring expressions that can be jointly understood, so children were provided scaffolding around taking turns and talking to their partner. Thus, children's performance should be taken as a proof-of-concept about ability, rather than a claim about how generally children spontaneously demonstrate these abilities. 

Even within this population, we observed some substantial pair to pair differences. Pairs varied in what types of descriptions used, how readily they produced them, as well as how well they understood the "rules" of the game and how much they cooperated with the rules.

Our qualitative impression is that it was more common for children to hit limits around understanding the game structure (ex. that the "teller" doesn't press the iPad),  cooperating with the game (ex. not turning their iPad to show their partner, not pressing buttons before their partner has a chance to describe), and understanding that this wasn't a test about the "name" of the shape, than for children to specifically struggle with how to describe an unknown referent adequately.
Pairs of children did vary more in how sensitive they were to collaborating with their partner and adapting descriptions based on their partners descriptions and their prior success. Generally, it's hard to push at the lower limits of some of children's abilities because many skills are changing over the same time frame and variables that may not be of primary interest, such as patience and instruction-following, can influence the outcomes. [TODO do we want to explicitly comment that we aren't making claims about what the lower limit is]
