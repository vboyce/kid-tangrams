...
Some examples to illustrate the variety of effective descriptions children employed are shown in Table \ref{example} (these specific examples are from Experiment 2, but both experiments had similar distributions of descriptions). TODO add examples from expt 3 as well??
...
```{r age-acc, fig.env="figure", fig.pos = "t", fig.align = "center", out.width="100%", fig.width=5, fig.height=3, fig.cap = "Accuracy of selections across all experiments, graphed by the mean age of child in that game, and colored by the number of girls in the pair. Each dot is a game, and linear fits are overlayed. \\label{agewords}" }
joint_data |>
...
For description length ... TODO 

These findings of a slight effect of age and a slight female-advantage are consistent with a wider literature of ... 
...
that said, even when children were struggling, some of their attempts such as gesturing or turning ipads or asking (can you show me) are indicative of attempts at conversational monitoring and solving the communication problem (albeit not in ways we that followed the "rules" of the game)
