...
```{r}
# for use in abstract 
pairs <- joint_data |> distinct(game, expt) |> nrow()
# eventually expect 81

pct <- joint_data |> summarize(m=round(mean(correct)*100)) |> pluck()
```
...
```{r description, fig.env="figure", fig.pos = "t", fig.align = "center", out.width="100%", fig.width=7, fig.height=3, fig.cap = "Length of description produced by the teller each trial, shown on a log scale. Grey dots are individual data points, colored dots are per trial means with bootstrapped 95% CIs. \\label{length}" }
joint_data |> 
...
```{r sims, fig.env="figure", fig.pos = "t", fig.align = "center", out.width="101%", fig.width=7, fig.height=3, fig.cap = "Semantic similarity between pairs of descriptions from different sources. Dots are means and lines are bootstrapped 95% CIs.  \\label{sim}" }
...
## Methods
As Experiment 2 was very similar to Experiment 2, we focus on the changes made compared to Experiment 2. Experiment 3 was pre-registered at [anonymized link] (https://osf.io/tk7sc/overview?view_only=874df1897c2e45e187aa71944bbc2392). 

The procedure for Experiment 3 matched Experiment 2, and the only change was in the stimuli used.


### Participants
Experiment 3 was run between  and YY, at the same preschool as Experiments 1 and 2. Because the stimuli changed, we allowed children who had participated in a prior experiment to participate again, and TODO participants in experiment 2 were also in experiment 3. TODO pairs of children completed all 16 critical trials, and TODO others completed ... . Our target age range was 4 and 5-year-olds. Of the TODO children, TODO were girls, and the children had a median age of TODO months, and a range of TODO-TODO months. An additional TODO games were excluded for being pilot runs, including repeat participants, not completing enough trials, or including a 3-year-old participant.  
...
As the experiments were all similar to one another, we re-ran models pooling the data across the three experiments, with experiment number as a random effect.

### Accuracy and speed 
Pooling across the three experiments, children's accuracy was above chance (OR: `r stats_text(acc_mod_meta,1)`) and accuracy numerically increased over the course of the game, although the credible interval included 0 (OR of one trial later: `r stats_text(acc_mod_meta, 2)`). Across all the experiments, children sped up over repetitions (effect of one trial later: `r stats_text(speed_mod_meta, 2)` seconds).
...
Children in the late preschooler years are undergoing a time of rapidly developing linguistic and cognitive skills, so children may show age-based differences in how well they complete tasks that use language or executive functioning [TODO citation needed]. Working memory and other executive capacities are developing across childhood [@diamond2013]. Additionally, there may be gender differences at this age. Young girls have on average an advantage in early linguistic skills, showing faster earlier vocabulary growth that boys [@marchman; @rinaldi2023]. TODO needs work
...
It is curious that guesser gender but not teller gender affected description length, but for this task, it is unclear how description length would be expected to map onto linguistic sophistication for this task and demographic, since short labels could be non-specific like "person", or adequately distinguishing like "mountain".

# General discussion

Prominent early studies claimed that young children cannot overcome their ego-centrism to coordinate with each other in reference games [@glucksberg1966]. However, more recent developmental work has found that young children show emerging communicative and pragmatic sensitivity, especially when cognitive demands are low. Recent work has established that children as young as 4 years old are successful at producing and understanding referential expressions to novel shapes when playing reference games with their parents [@leung2024]. Here, we revisited the question of preschoolers' performance in child-child reference games, using a scaffolded paradigm to reduce extraneous task demands. 
...
## Limitations
The generalizability of our results is limited by the target population, the target images, and the task structure. We sampled a convenience population of children at a university nursery school. We used two sets of 4 tangram images, which may be easier to distinguish and have higher codability than other target images used in adult iterated reference games. We specifically targeted children's abilities to construct referring expressions that can be jointly understood, so children were provided scaffolding around taking turns and talking to their partner. Thus, children's performance should be taken as a proof-of-concept about ability, rather than a claim about how generally children spontaneously demonstrate these abilities. 

## Qualitative observations
Even within this population, we observed substantial pair to pair differences in what types of descriptions were used, how readily they produced them, and how well children understood and cooperated with the "rules" of the game.
