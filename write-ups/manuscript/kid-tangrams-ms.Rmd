...
TODO all figures could be smaller!
...
TODO update diagram with expt 3 stim.
...
* [@sanjuan2025] When 4-5 year old children learn that some objects are only visible to them and not their partner, they are faster to rule out these competitors and select a target than when they believe the interlocuter is aware of the competitors as well. 
* [@nilsen2009] take into account partners perspective for both speaker and listening (4-5 yos); 3-4 year olds also on listening only; relate this to inhibitory control. 
...
- Bohn, Kachel, Tomasello (2019) Young Children spontaneously recreate core properties of language in a new modality
    
    - 4, 6, (and 8 in a separate thing) communicating target image via video link (no audio after practice trials)
        
    - comprehension good and increasing, gestures used back by second partner when roles switch
        
    - (out of 5 pictures, including “blank”)
        
    - do see more similaritiy w/i dyads especially for 6 versus 4 yos
    
May want to set up for change over age? And a little more on why 4-5 is a good age -- it's a time of rapid development, we think from prior work that children can comprehend the task. Just before big changes in linguistic input from school + reading. ... 

Given the task demands in @glucksberg1966 and work showing that children can succeed in a less demanding paradigm, here we revisit the question of child-child referential communication. In the present study, we re-examine young children's ability to establish effective referring expressions with each other in an iterated reference game using a simplified tablet-based paradigm. Across three studies including a total of ZZ pairs of 4-5-year-old children, we found that preschool-aged children were successful in an iterated reference game, suggesting that children's capacity to construct effective referring expressions in novel contexts emerges earlier than once claimed.
...
TODO UPDATE EXAMPLES TO HAVE SOME FROM EXPT 3
...
```{r sim2, fig.env="figure", fig.pos = "t", fig.align = "center", out.width="100%", fig.width=5, fig.height=3, fig.cap = "Semantic similarity between descriptions from earlier blocks (1-3) and the last block in Experiments 2 and 3.  Heavy dots are means with bootstrapped 95% CIs; light dots are individual values. \\label{tolast}" }
joint_similarities |> 
  filter(game1==game2) |> 
  filter(later==3) |> 
...
We did not account for children's age and gender when pairing children to play the game, so children of different ages (within the 4-5 year old target age range) sometimes played together, and children sometimes played with same-gender and sometimes with different-gender partners. As children of this age are rapidly developing their linguistic skills (CITATION NEEDED), we looked at possible effects of children's age and gender on performance as an exploratory measure. 

```{r age-acc, fig.env="figure", fig.pos = "t", fig.align = "center", out.width="100%", fig.width=5, fig.height=3, fig.cap = "Accuracy of selections across all experiments, graphed by the mean age of child in that game, and colored by the number of girls in the pair. Each dot is a game, and linear fits are overlayed. \\label{ageacc}" }
joint_data |>
...
```{r age-words, fig.env="figure", fig.pos = "t", fig.align = "center", out.width="100%", fig.width=5, fig.height=3, fig.cap = "Number of words of description produced by the teller across all experiments, graphed by the age of teller in that game, and colored by teller gender. Each dot is a game, and linear fits are overlayed. \\label{agewords}" }
