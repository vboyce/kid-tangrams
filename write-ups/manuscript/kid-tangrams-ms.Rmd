...
Learning a language requires learning not only the content of that language, but also how to use the language to communicate. One case study for language use is referential communication, the ability to describe a target so an interlocutor can pick it out from a set of possibilities. Adults show sensitivity to both the visual context and their audience during referential communication, calibrating the description they provide to their beliefs about the interlocutor's knowledge state TODO CITATIONS FROM THE ADULT WORK. 
...
TODO TABLE ADD WHAT ROUNDS (blocks) THEY COME FROM (can look up easily)
...
TODO emphasize that this is to maximize power
TODO should we have random slopes as well as random intercepts? (re-run models and re-propagate?)
...
While some of these effects were small and thus not all effects were consistently detected in every experiment, the overall pattern of results is consistent in direction with the patterns of convergence and divergence reported in reference games between adults [@boyce2024]. 


## Sensitivity to prior result

TODO foreshadow this result, possibly even in intro?
...
TODO reframe for this position in the MS
...
TODO start with the major question 
* when does the ability to communicate in a partner- and context-sensitive way arise? There are different components to communication, but ... TODO
...
TODO REFACTOR INTO 
PARA 1 -- we don't find shortening, but it may be that they're already short 

PARA 2 -- the semantic signals recapitulate trends found in adults -- children show similarity to within-game / some signs of convention formation / some sensitivity to errors 
...
TODO if not noted elsewhere:
* note that the slight increase in accuracy is quite different from the towards ceiling effects common in adults
* interpretation of semantic lack of divergence
I had a few suggestions for organization of GD and joint analysis, but they should be very easy to implement.[6:02 AM]
I would cut the framing of the age and gender. Just analyze them and discuss briefly in discussion.
[6:03 AM]your interpretation is not strong at all I would say, you primarily focus on the most obvious result, which is the high accuracy.
[6:04 AM]so I wouldn’t worry too much about the interpretive stuff? you have a clause in the abstract about the semantic analyses, which seemed appropriate.
