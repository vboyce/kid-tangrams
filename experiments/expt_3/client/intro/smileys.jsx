...
export default class Smileys extends React.Component {
  constructor(props) {
    ...
    this.bubbleInfo = [];
    dotx.forEach((x, i) =>
      this.bubbleInfo.push({ x: x, y: doty[i], i: i + 1 })
    );
  }
  ...
  render() {
    ...
    let allBubbles;
    const check_all_popped = () => {
      if (this.state.count == this.dotCount) {
        setTimeout(function () {
          onNext();
        }, 1000);
      }
    ...
    const popped = () => {
      this.setState({ count: this.state.count + 1 }, check_all_popped);
    };
    ...
    return (
      <Centered>
        <div className="instructions">{allBubbles}</div>
      </Centered>
    );
  }
}
